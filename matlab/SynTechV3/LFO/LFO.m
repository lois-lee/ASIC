...
sta.LFO.Max_Min_I = 0.032258; 
% to be reassigned to read the division value from a outside block
...
% compute increment (LFO does not reset at keystroke)
sta.LFO.Inc_D = RealRESIZE(par.LFO.Freq_DI/par.GLO.FSInt_DI,FixP_par,QType_out);

% increment position
sta.LFO.Pos_D = RealRESIZE(sta.LFO.Pos_D + sta.LFO.Inc_D, FixP_par, QType_out);

% reset position if mode = 1 and on keypress
if par.LFO.Rst_Mode_S == 1 && (sta.LFO.Key_DP ~= sta.INP.Key_DI)
    sta.LFO.Pos_D = 0;
end
...
elseif par.LFO.Wave_S == 2 % rand
    if sta.LFO.Pos_D < sta.LFO.Inc_D % check for new period
        ...
        t = [5 2];
        seq = LFO_LFSR(s, t);        
        Tmp_D = LFO_mapping(bi2de(seq), 0, 65535, -1, 1, sta.LFO.Max_Min_I); % randomize
