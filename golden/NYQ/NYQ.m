...
FixP_sum = {0,46,'s'};
QType_sum = 'WrpTrc_NoWarn';

FixP_prod = {0,46,'s'};
QType_prod = 'WrpTrc_NoWarn';

FixP_acc = {0,46,'s'};
QType_acc = 'WrpTrc_NoWarn';
...
sta.NYQ.ins = [sta.NYQ.ins; sta.NYQ.Sample_D];
% disp(sta.NYQ.Sample_D);
...
for i=1:length(sta.NYQ.accumulate_D)
    sta.NYQ.accumulate_D(i) = RealADD(sta.NYQ.accumulate_D(i), ...
        RealMULT(sta.NYQ.coeff(sta.GLO.OSR_DO * (i) - (sta.NYQ.Cnt_D - 1)), ...
        sta.NYQ.Sample_D, FixP_prod, QType_prod), FixP_sum, QType_sum);
    % real resize the accumulates
    sta.NYQ.accumulate_D(i) = RealRESIZE(sta.NYQ.accumulate_D(i),...
        FixP_acc, QType_acc); 
    
    
%     if sta.NYQ.Sample_D>0 
%      keyboard
%     end
end
...
%             if(sta.NYQ.accumulate_D(i+1)>0)
%                 disp(sta.NYQ.accumulate_D(i+1))
%                 keyboard
%             end
            % real resize the temp ff
            % sta.NYQ.temp_D(i) = RealRESIZE(sta.NYQ.temp_D(i),...
...
sta.NYQ.outs = [sta.NYQ.outs; sta.NYQ.Out_DO];
% =========================================================================
% ================         PERFORMANCE CHECKER          =================== 
% =========================================================================
